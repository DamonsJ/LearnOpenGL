...
Camera::Camera(void) { updateCameraVectors(); }
void Camera::setCameraDistance(float d) {
  m_distance = d;
  updateCameraVectors();
}
void Camera::updateCameraVectors(void) {
  m_position_coords = glm::vec3(glm::cos(glm::radians(m_position_xangle)) *
                                    glm::cos(glm::radians(m_position_yangle)),
                                glm::sin(glm::radians(m_position_yangle)),
                                glm::sin(glm::radians(m_position_xangle)) *
                                    glm::cos(glm::radians(m_position_yangle)));
...
void Camera::setSensitivities(float mouse_sensitivity, float zoom_sensitivity,
