...
public:
    ...
    sherwood_v3_table()
    {}
    explicit sherwood_v3_table(uint64_type          bucket_count,
    ...
    {}
    sherwood_v3_table(uint64_type          bucket_count,
    ...
    void rehash(uint64_t num_buckets)
    {
        ...
        EntryPointer special_end_item = new_buckets +
...
struct prime_number_hash_policy
{
    ...
    mod_function next_size_over(uint64_t& size) const
    {
        ...
        // you now have large gaps which you would hit if somebody called reserve() with an unlucky
        // number.
        // 4. to fill the gaps for every prime p go to wolfram alpha and get ClosestPrime(p *
        // 2^(1/3)) and ClosestPrime(p * 2^(2/3)) and put those in the gaps
        ...
        const uint64_t* found = std::lower_bound(std::begin(prime_list),
                                                 std::end(prime_list) - 1,
                                                 size);
        size                  = *found;
...
{
    typedef DGL::power_of_two_hash_policy hash_policy;
};
